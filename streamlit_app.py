...
.sec-hdr{color:#e6edf3;font-weight:700;font-size:.97rem;margin:12px 0 6px;letter-spacing:.01em;}
.sub-hdr{color:#c9d1d9;font-weight:600;font-size:.88rem;margin:8px 0 4px;}
hr.thin{border:none;border-top:1px solid #21262d;margin:14px 0;}
.ok-card{background:#0d2218;border:1px solid #238636;border-radius:6px;padding:8px 14px;color:#86efac !important;font-size:.82rem;margin:4px 0;}
.warn-card{background:#2d2000;border:1px solid #d29922;border-radius:6px;padding:8px 14px;color:#f0c040 !important;font-size:.82rem;margin:4px 0;}
.err-card{background:#2d1515;border:1px solid #da3633;border-radius:6px;padding:8px 14px;color:#fca5a5 !important;font-size:.82rem;margin:4px 0;}
.info-card{background:#0c1a2e;border:1px solid #1f6feb;border-radius:6px;padding:8px 14px;color:#79c0ff !important;font-size:.82rem;margin:4px 0;}
.badge-green{background:#0d2218;border:1px solid #238636;border-radius:12px;padding:2px 9px;color:#86efac !important;font-size:.74rem;}
.badge-yellow{background:#2d2000;border:1px solid #d29922;border-radius:12px;padding:2px 9px;color:#f0c040 !important;font-size:.74rem;}
.badge-red{background:#2d1515;border:1px solid #da3633;border-radius:12px;padding:2px 9px;color:#fca5a5 !important;font-size:.74rem;}
.badge-grey{background:#161b22;border:1px solid #30363d;border-radius:12px;padding:2px 9px;color:#8b949e !important;font-size:.74rem;}
[data-testid="stFileUploader"]{background:#0d1117 !important;border:1px dashed #30363d !important;border-radius:8px !important;}
[data-testid="stFileUploader"] label{color:#8b949e !important;}
div[data-testid="stSelectbox"] label,div[data-testid="stTextInput"] label,
div[data-testid="stNumberInput"] label,div[data-testid="stDateInput"] label{color:#8b949e !important;font-size:.82rem !important;}
div[data-testid="stSelectbox"] > div > div,div[data-testid="stTextInput"] input,
div[data-testid="stNumberInput"] input{background:#010409 !important;border:1px solid #30363d !important;color:#c9d1d9 !important;border-radius:6px !important;}
div[data-testid="stSelectbox"] svg{fill:#8b949e !important;}
.stButton > button{background:#161b22 !important;border:1px solid #30363d !important;color:#c9d1d9 !important;border-radius:6px !important;font-size:.85rem !important;padding:6px 16px !important;transition:all .15s !important;}
.stButton > button:hover{background:#21262d !important;border-color:#6366f1 !important;color:#e6edf3 !important;}
...
.stDownloadButton > button{background:#0d2218 !important;border:1px solid #238636 !important;color:#86efac !important;border-radius:6px !important;font-size:.82rem !important;}
.stDownloadButton > button:hover{background:#133d25 !important;}
...
[data-testid="stExpander"]{border:1px solid #21262d !important;border-radius:8px !important;background:#0d1117 !important;}
[data-testid="stExpanderHeader"]{color:#c9d1d9 !important;font-size:.87rem !important;}
.stCheckbox label{color:#c9d1d9 !important;font-size:.85rem !important;}
.int-card{background:#0c1520;border:1px solid #1f3a5f;border-radius:6px;padding:8px 12px;margin:4px 0;}
.alloc-card{background:#0d1117;border:1px solid #21262d;border-radius:10px;padding:14px 16px;margin:8px 0;}
</style>
...
def ext_suggestions(panel_row, sf, ssmap):
    """External staff: 🟢 Willing (SubjectMap), 🟡 Same-Dept, ⚪ Others — sorted least duties first in each group"""
    ...
    for _,row in sf.iterrows():
        ...
        if sid in mapped_ids:
            entry["group"]="willing"; entry["icon"]="🟢"; entry["cls"]="badge-green"
            willing.append(entry)
        elif dep == p_dep:
            entry["group"]="same_dep"; entry["icon"]="🟡"; entry["cls"]="badge-yellow"
            same_dep.append(entry)
        else:
            entry["group"]="other"; entry["icon"]="⚪"; entry["cls"]="badge-grey"
            others.append(entry)
...
def make_dropdown_label(s):
    """Format: 🟢/🟡/⚪ INSTT-DepCode | StaffID-Name | Designation | Duties:X"""
    return f"{s['icon']} {s['instt']}-{s['dep']} | {s['sid']}-{s['name']} | {s['desig']} | Duties:{s['count']}"

def extract_sid(label):
    """Extract staff ID from label: icon INSTT-Dep | SID-Name | Desig | Duties:X"""
    l = str(label).strip()
    l = re.sub(r'^[🟢🟡🔴⚪]\s*','',l)
    if "|" in l:
        parts = l.split("|")
        if len(parts) >= 2:
            sid_name = parts[1].strip()   # "STAFFID-Name"
            if "-" in sid_name:
...
def generate_pdf_rl(panel_df, sf, submap):
    ...
    for sid in sorted(sd.keys(),key=lambda s:get_name(sf,s)):
        ...
        ht=Table([[" Staff ID",sid," Name",name],
        ...
        dt=Table(tr,
                 colWidths=[.9*cm,2*cm,2*cm,4*cm,1.2*cm,2.2*cm,3.5*cm,2.2*cm,2*cm,2*cm],
                 repeatRows=1)
...
def generate_html_duties(panel_df, sf, submap):
    ...
    for sid in sorted(sd.keys(),key=lambda s:get_name(sf,s)):
        ...
        for sno,d in enumerate(duties,1):
            ...
            pn=get_name(sf,pid)  if pid else "-"
            pp=get_phone(sf,pid) if pid else "-"
            rows+=(f"<tr><td>{sno}</td><td>{d['ins']}</td><td>{d['sc']}</td>"
                   f"<td>{d['sn'] or d['sc']}</td><td>{d['role']}</td>"
                   f"<td>{pid or '-'}</td><td>{pn}</td><td>{pp}</td>"
                   f"<td style='min-width:80px'></td><td style='min-width:80px'></td></tr>")
        pages.append(f"""
<div style="page-break-after:always;font-family:Arial,sans-serif;padding:20px;max-width:900px;margin:0 auto">
<h2 style="text-align:center;color:#1a1a2e">PRACTICAL EXAM DUTY ORDER</h2>
<p style="text-align:center;color:#666;font-size:11px">{CREATOR}</p>
<table border="1" cellpadding="6" cellspacing="0" style="width:100%;border-collapse:collapse;margin:10px 0">
...
<tr><th>S.No</th><th>INSCODE</th><th>SubCode</th><th>Subject Name</th><th>Role</th>
<th>Partner ID</th><th>Partner Name</th><th>Partner Phone</th><th>Date From</th><th>Date To</th></tr>
</thead><tbody>{rows}</tbody></table>
<p style="font-size:10px;color:#666">Date From/To to be filled by Flying Squad at duty.</p>
</div>""")
    html=("<!DOCTYPE html><html><head><meta charset='utf-8'>"
          "<title>Duty Sheets</title></head><body>"+"".join(pages)+"</body></html>")
...
filled_c = (st.session_state.panel["EXTID"].apply(norm_id)!="").sum() if panel_c else 0
...
<div class="topbar">
  <div style="display:flex;align-items:center;gap:12px">
    ...
    <div>
      <div class="tb-title">Duty Manager v4</div>
...
<div class="statsbar">
  ...
  <div class="sc">🗓️ Dated <b>{pdate_c}</b></div>
...
# ═══════════════════════════════════════════════════════
with tab_up:
    st.markdown('<div class="sec-hdr">📥 Upload Centre — Panel, Staff, Subject Maps</div>',unsafe_allow_html=True)

    u1,u2=st.columns([1,1],gap="medium")
    ...
    with u1:
        ...
        st.markdown('<span class="sub-hdr">📖 Current Panel</span>',unsafe_allow_html=True)
        ...
        st.dataframe(pv[show_p].fillna(""),use_container_width=True,height=300)
    ...
    with u2:
        ...
        st.markdown('<hr class="thin">',unsafe_allow_html=True)
        st.markdown('<span class="sub-hdr">👥 Current Staff</span>',unsafe_allow_html=True)
        sf_show=[c for c in ["Staff ID","INSTT","Name of the Staff","dep code","Designation"] if c in st.session_state.staff.columns]
        st.dataframe(st.session_state.staff[sf_show].fillna(""),use_container_width=True,height=300)
    ...
    # ── Subject Map ──
    with s1:
        st.markdown('<span class="sub-hdr">📘 Subject-Staff Map CSV (SubjectMap)</span>',unsafe_allow_html=True)
        ...
        if not st.session_state.ssmap.empty:
            st.dataframe(st.session_state.ssmap[SMAP_COLS].fillna(""),use_container_width=True,height=200)
        else:
            st.markdown('<div class="info-card">ℹ️ No SubjectMap loaded. Auto-allocate will use 🟡 Same-Dept only.</div>',unsafe_allow_html=True)

    # ── SubCode Map ──
    with s2:
        st.markdown('<span class="sub-hdr">📗 SubCode Name Map CSV</span>',unsafe_allow_html=True)
        ...
        if not st.session_state.submap.empty:
            st.dataframe(st.session_state.submap.fillna(""),use_container_width=True,height=200)

# ═══════════════════════════════════════════════════════
# TAB 2 — EXT ALLOCATE (with sub-tabs)
# ═══════════════════════════════════════════════════════
with tab_ext:
    ...
    m1.metric("📋 Pending EXTID", len(all_cands))
    ...
    # ════════════════════════════════════════
    # SUB-TAB A — AUTO ALLOCATE
    # ════════════════════════════════════════
    with sub_auto:
        with st.expander("ℹ️ Allocation Rules"):
            st.markdown("""
| # | Rule | Detail |
|---|------|--------|
| 1 | **SubjectMap** | Matches SUBCODE via Subject-Staff Mapping |
| 2 | **External Rule** | Staff INSTT must differ from panel INSCODE |
| 3 | **Priority** | 🟢 Willing (SubjectMap) → 🟡 Same Dept → ⚪ Others; least duties first |
            """)
        ...
        if not view_panel.empty:
            ...
            show_cols=[c for c in ["STATUS","INSCODE","NCNO","SUBCODE","SUBNAME",
                                   "NOC","INTID","INT_NAME","EXTID","EXT_NAME"] if c in pv.columns]
    ...
    # ════════════════════════════════════════
    # SUB-TAB B — MANUAL ALLOCATION (card view)
    # ════════════════════════════════════════
    with sub_manual:
...
<div class="alloc-card">
  <!-- Row 1: INSCODE · NCNO · SUBCODE · Subject Name -->
  <div style="display:flex;flex-wrap:wrap;gap:8px;align-items:center;margin-bottom:10px">
    <span style="background:#1c2333;border:1px solid #30363d;border-radius:4px;
                 padding:3px 10px;font-size:.82rem;color:#79c0ff">🏫 {ins}</span>
    <span style="background:#1c2333;border:1px solid #30363d;border-radius:4px;
                 padding:3px 10px;font-size:.82rem;color:#d2a8ff">🏭 {nc}</span>
    <code style="background:#010409;padding:3px 10px;border-radius:4px;
                 font-size:.84rem;color:#f0883e">{sc}</code>
    {"<span style='font-size:.8rem;color:#8b949e'>"+sn+"</span>" if sn else ""}
    <span style="margin-left:auto;font-size:.78rem;color:#8b949e">
      👥 {noc} students &nbsp;·&nbsp; Batches: {nob}
    </span>
  </div>
  <!-- Row 2: INTID - Name (prominent) -->
  <div style="background:#010409;border-radius:6px;padding:8px 12px;margin-bottom:8px;
              display:flex;flex-wrap:wrap;gap:10px;align-items:center">
    <span style="font-size:.76rem;color:#6e7681;white-space:nowrap">🎓 Internal Examiner:</span>
    {"<code style='background:#1c2333;padding:2px 8px;border-radius:4px;color:#fbbf24;font-size:.86rem'>"+intid+"</code>" if intid else "<span style='color:#6e7681;font-size:.8rem'>No INTID assigned</span>"}
    {"<span style='color:#e6edf3;font-size:.88rem;font-weight:700'>&nbsp;"+intname+"</span>" if intname else ""}
    {"<span style='color:#8b949e;font-size:.78rem'>&nbsp;·&nbsp;"+int_desig+"</span>" if int_desig else ""}
    {"<span style='color:#6e7681;font-size:.76rem'>&nbsp;·&nbsp;📞 "+int_phone+"</span>" if int_phone else ""}
  </div>
</div>
""", unsafe_allow_html=True)
                    ...
                    # ── Status badge ──
                    if cur_ext:
                        ext_name=get_name(sf,cur_ext); ext_desig=get_desig(sf,cur_ext); ext_ph=get_phone(sf,cur_ext)
                        st.markdown(
                            f'<div class="ok-card" style="padding:7px 14px;margin-bottom:6px">'
                            f'✅ <b>EXTID: {cur_ext}</b> — {ext_name}'
                            f'{"&nbsp;·&nbsp;"+ext_desig if ext_desig else ""}'
                            f'{"&nbsp;📞 "+ext_ph if ext_ph else ""}'
                            f'</div>',unsafe_allow_html=True)
                    elif sv_val:
                        sv_id=extract_sid(sv_val) if "|" in sv_val else norm_id(sv_val)
                        sv_name=get_name(sf,sv_id)
                        st.markdown(
                            f'<div class="warn-card" style="padding:7px 14px;margin-bottom:6px">'
                            f'🟡 Staged (not saved): <b>{sv_id}</b>'
                            f'{"&nbsp;—&nbsp;"+sv_name if sv_name else ""}'
                            f'</div>',unsafe_allow_html=True)
                    else:
                        st.markdown('<div class="err-card" style="padding:5px 14px;margin-bottom:6px">⏳ Not assigned yet</div>',unsafe_allow_html=True)

                    # ── Legend ──
                    st.markdown(
                        f'<div style="font-size:.74rem;color:#6e7681;margin-bottom:3px">'
                        f'🟢 {w_cnt} Willing (SubjectMap) &nbsp;&nbsp; 🟡 {sd_cnt} Same-Dept &nbsp;&nbsp; ⚪ {ot_cnt} Others &nbsp;&nbsp; (least duties listed first in each group)'
                        f'</div>',unsafe_allow_html=True)
                    ...
                    sel     = st.selectbox(
                        f"🔽 External Examiner — {len(suggs)} available",
                        s_labels, index=di, key=f"sel_{pidx}",
                        help="🟢 Willing (in SubjectMap for this subject) | 🟡 Same Dept | ⚪ Others | Sorted by least duties within each group")

                    # ── Manual input + Apply ──
                    mc1,mc2 = st.columns([5,1])
                    man = mc1.text_input(
                        "", value="", key=f"man_{pidx}",
                        placeholder="✏️ Or type Staff ID manually (overrides dropdown)",
                        label_visibility="collapsed",
                        help="Manually enter a Staff ID if not in dropdown")
                    ...
                    if sel and sel!=s_labels[0]:
                        ...
                        st.markdown(
                            f'<div style="background:#0c1a2e;border-left:3px solid #6366f1;'
                            ...
                            f'{"<span style=color:#6e7681>Dept: "+dep_s+"</span>" if dep_s else ""}'
    ...
    # ════════════════════════════════════════
    # SUB-TAB C — DOWNLOAD (inside EXT Alloc)
    # ════════════════════════════════════════
    with sub_dl_ext:
        st.markdown('<div class="sec-hdr">📥 Download Allocated Panel Data</div>',unsafe_allow_html=True)
        ...
        with dd1:
            st.download_button(
                ...
                mime="text/csv", use_container_width=True, key="dl_alloc")
        with dd2:
            st.download_button(
        ...
        st.markdown('<hr class="thin">',unsafe_allow_html=True)
        st.markdown('<div class="sec-hdr">🖨️ PDF Duty Sheets</div>',unsafe_allow_html=True)

        if dl_filled.empty:
            st.markdown('<div class="warn-card">⚠️ No filled EXTID rows yet. Assign EXTID first, then download PDF here.</div>',unsafe_allow_html=True)
        else:
            st.markdown(f'<div class="info-card">📄 Will generate duty sheets for <b>{len(dl_filled)}</b> allocated rows</div>',unsafe_allow_html=True)
            if RPDF:
                if st.button("⚙️ Generate PDF Duty Sheets",type="primary",key="gen_pdf_ext",use_container_width=True):
                    with st.spinner("Building PDF..."):
                        try:
                            pdf_b=generate_pdf_rl(dl_filled,dl_sf,dl_sub)
                            st.download_button(
                                "📄 Download PDF Duty Sheets",
                                data=pdf_b,
                                file_name=f"duty_sheets{'_'+dl_ins if dl_ins!='All' else ''}.pdf",
                                mime="application/pdf",
                                use_container_width=True, key="dl_pdf_ext")
                            st.success("✅ PDF ready!")
                        except Exception as e:
                            st.error(f"❌ PDF error: {e}")
            else:
                st.markdown('<div class="warn-card">⚠️ reportlab not installed. Check requirements.txt.</div>',unsafe_allow_html=True)
...
with tab_dl:
    ...
    if all_p.empty:
        ...
        with dl_full_cols[1]:
            ...
            st.download_button(
                label=f"📥 Panel + SUBNAME — {len(pf2)} rows",
        ...
        ins_chunks=[inscodes_p[i:i+4] for i in range(0,len(inscodes_p),4)]
    ...
    if all_p.empty:
        ...
        pdf_ins_f=st.selectbox("🏫 Filter by INSCODE for PDF",
        ...
        st.markdown(f'<div class="info-card">📄 <b>{len(pdf_filled)}</b> rows with EXTID filled (out of {len(pdf_data)} total)</div>',unsafe_allow_html=True)

        if RPDF:
            if st.button("⚙️ Generate PDF Duty Sheets",type="primary",
                         use_container_width=True, key="gen_pdf_main"):
                with st.spinner("Building PDF... (may take a moment)"):
                    try:
                        pdf_b=generate_pdf_rl(pdf_filled if not pdf_filled.empty else pdf_data,
                                              sf_dl, sub_dl)
                        st.download_button(
                            "📄 Download PDF Duty Sheets",
                            data=pdf_b,
            ...
            st.markdown('<div class="warn-card">⚠️ reportlab not found. Verify requirements.txt includes <code>reportlab</code>.</div>',unsafe_allow_html=True)
